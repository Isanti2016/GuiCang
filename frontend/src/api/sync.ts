...
export function createTask(data: { name: string; sourceConfig: string; cron: string }): Promise<SyncTask> {
  return post<SyncTask>('/sync/tasks', data)
}

export function updateTask(id: number, enabled: boolean, data: { name: string; sourceConfig: string; cron: string }): Promise<SyncTask> {
...
export function fetchHistory(taskId?: number, limit = 50): Promise<SyncHistory[]> {
