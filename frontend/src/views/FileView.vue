...
// ---------- 目录树 ----------
const treeData = ref<FileEntry[]>([{ name: '根目录', path: '', dir: true, size: 0, mtime: 0, kind: 'dir' }])
...
async function loadTree(): Promise<void> {
  treeData.value = [{ name: '根目录', path: '', dir: true, size: 0, mtime: 0, kind: 'dir' }]
}
  ...
  <el-container class="file-manager">
    ...
    <el-main class="file-manager__main">
      <div class="file-manager__toolbar">
        ...
        <div class="file-manager__actions">
          ...
          <el-button size="small" type="primary" @click="uploadDialog = true">上传</el-button>
        </div>
      </div>

      <el-table v-loading="loading" :data="list" size="small" @row-dblclick="openPreview">
        ...
        <el-table-column prop="name" label="名称" min-width="220" show-overflow-tooltip>
          <template #default="{ row }">
            <span class="file-manager__name" @click="row.dir ? navigate(row.path) : canPreview(row) && openPreview(row)">
              {{ row.name }}
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="240">
          <template #default="{ row }">
            <el-button link type="primary" size="small" @click="openPreview(row)" :disabled="!canPreview(row)">
              预览
            </el-button>
            ...
            <el-button
              ...
              type="primary"
              size="small"
              @click="(renameTarget = row), (renameForm.name = row.name), (renameDialog = true)"
            >
              重命名
            </el-button>
    ...
    <el-drawer v-model="previewOpen" :title="previewEntry?.name" size="60%">
      <template v-if="previewEntry">
        ...
        <video v-else-if="previewEntry.kind === 'video'" :src="streamUrl(previewEntry.path)" controls style="width: 100%" />
    ...
    <el-dialog v-model="uploadDialog" title="上传文件" width="500px">
      <el-upload v-model:file-list="uploadFiles" drag multiple :auto-upload="false">
    ...
    <el-dialog v-model="moveDialog" title="移动到目录" width="500px">
      <div class="file-manager__move-toolbar">
        <el-button size="small" @click="moveDir = ''; refreshMoveCandidates()">根目录</el-button>
        <el-button
          size="small"
          :disabled="!moveDir"
          @click="moveDir = moveDir.includes('/') ? moveDir.substring(0, moveDir.lastIndexOf('/')) : ''; refreshMoveCandidates()"
        >
          上一级
        </el-button>
      </div>
      <el-scrollbar max-height="300px">
        <div
          ...
          @click="moveDir = dir.path; refreshMoveCandidates()"
        >
          <el-icon color="#409eff"><Folder /></el-icon>
          <span>{{ dir.name }}</span>
        </div>
        <el-empty v-if="moveCandidates.filter((d) => d.dir).length === 0" description="当前目录无子目录" :image-size="60" />
      </el-scrollbar>
      <div class="file-manager__move-target">目标：{{ moveDir || '（根目录）' }}</div>
      <template #footer>
