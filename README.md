...
- **账号与权限**：PAM 认证（宿主机系统账号）+ JWT；RBAC 角色/权限点；目录级 ACL；两步验证（TOTP，含 10 个一次性恢复码防锁死）；登录会话管理（在线设备列表 + 踢下线）
- **文件管理**：目录列表/新建/重命名/移动/删除；上传（≤1G 流式 + 大文件分片 >50MB + 断点续传）；下载/预览（HTTP Range 206）；批量打包下载（zip）；文件分享链接（密码/有效期）；md/txt 网页编辑 + 历史版本回滚；全文检索（名称 + 内容）；重复文件检测；文件收藏（星标）；图片/视频缩略图
...
- **PWA**：manifest + Service Worker（网络优先离线回退），手机浏览器可添加到主屏幕
